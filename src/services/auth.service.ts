import {ICredentials, IManager, ISeller, ITokenPair, ITokenPayload} from "../types";
import {Action, Manager, Seller, Token} from "../dataBase";
import {ApiError} from "../errors";
import {passwordService} from "./password.service";
import {tokenService} from "./token.service";
import {EActionTokenType, EEmailActions, ESmsActions, ETypesSeller} from "../enums";
...
class AuthService {
    ...
    public async login(credentials: ICredentials, user: ISeller | IManager): Promise<ITokenPair> {
        try {
    ...
    public async sendActivateToken(
        ...
        try {
            ...
            await emailService.sendMail("tatarkristina4@gmail.com", EEmailActions.CHANGE_TYPE_ACCOUNT, {
                token: actionToken
            });
        } catch (e) {
            throw new ApiError(e.message, e.status);
